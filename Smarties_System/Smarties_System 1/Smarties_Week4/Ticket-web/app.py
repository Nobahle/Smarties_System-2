...
# ---------------- CLASSIFICATION ----------------
def analyze_ticket_with_cohere(ticket_text):
    try:
        api_key = os.getenv("COHERE_API_KEY")
        if not api_key or api_key == "your_cohere_trial_key_here":
...
"""
        ...
        if dept not in DEPARTMENTS: dept = "Unrecognized"
        if tone not in ["Urgent", "Friendly", "Formal"]: tone = "Formal"
        
        return [dept], tone, risk, bias
    except Exception as e:
        print(f"[Fallback] Using keyword analysis due to Cohere API error: {e}")
...
def submit():
    ...
    categories, tone, risk, bias = analyze_ticket_with_cohere(ticket)
